  ...
  return <div className="min-h-screen">
    ...
    <header className="sticky top-0 z-20 border-b border-[#2A2A2A] bg-[#0A0A0A]/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center gap-5 px-4 py-3">
        ...
        {[['/admin/processos','Processos'],['/admin/dispositivos','Dispositivos'],['/admin/config','Configurações']].map(([href,label]) => <Link key={href} href={href} className={`text-sm ${path.startsWith(href) ? 'text-white' : 'text-zinc-400 hover:text-white'}`}>{label}</Link>)}
