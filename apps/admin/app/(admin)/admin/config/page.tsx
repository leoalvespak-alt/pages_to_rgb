...
export default function ConfigPage() {
  ...
  async function testProvider(name: string): Promise<void> {
    ...
    // S06.6: verifica a proposta atual do formulário (sem salvar antes).
    try {
      const proposed: Record<string, unknown> = { provider, model };
      if (keys[provider]?.trim()) proposed.api_key = keys[provider];
      if (provider === "google_document_ai") {
        proposed.google_document_ai_project_id = settings.google_document_ai_project_id;
        proposed.google_document_ai_location = settings.google_document_ai_location;
        proposed.google_document_ai_processor_id = settings.google_document_ai_processor_id;
        if (keys.google_document_ai?.trim()) proposed.google_document_ai_credentials = keys.google_document_ai;
      }
      const result = await apiFetch<{ ok: boolean; latency_ms: number; message?: string }>("/api/v1/admin/settings/test", { method: "POST", body: JSON.stringify(proposed) });
      setMessage(result.ok ? `${name}: conectado em ${result.latency_ms} ms (proposta verificada, nada salvo)` : `${name}: ${result.message || "falha"}`);
